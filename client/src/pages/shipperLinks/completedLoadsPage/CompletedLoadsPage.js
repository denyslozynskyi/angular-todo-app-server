...
import './completedLoadsPage.scss';

class CompletedLoadsPage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loads: [],
    ...
    componentDidMount = async () => {
        const loads = await serverConnection('/api/loads?status=shipped', 'GET', null, {Authorization: `Bearer ${this.state.token}`})
    ...
    render() {
        const loads = this.state.loads.map((load) => {            
            return (
                <div className="completed_load_card" key={load._id} data-key={load._id}>
        ...
        return (
            <div className='completed_load_cards'>
...
export default CompletedLoadsPage;